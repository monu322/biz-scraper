...
This is an answer without any markdown and it should be alright.`,
  ...
  return (
    <Box className="relative flex h-full min-h-[calc(100vh-12rem)] flex-col items-center gap-5">
            ...
            <Box className="flex flex-row gap-2">
              ...
              <Button
                ...
                endIcon={
                  <NiChevronRightSmall
                    size="small"
                    className={cn("-ml-1 transition-transform", versionSelectOpen && "rotate-90")}
                  />
                }
              >
      ...
      <Box
        className={cn(
          "flex w-full max-w-[50rem] flex-1 flex-col items-center justify-center pb-32 sm:px-4",
      ...
      >
        {conversation.length === 0 ? (
          <Box className="flex flex-col items-center gap-4">
            <Typography
              variant="h1"
              className="from-primary-dark via-primary to-primary-light inline-block bg-gradient-to-r bg-clip-text text-center text-transparent"
            >
      ...
      <Box className="bg-background fixed bottom-0 z-[2] w-full p-4 sm:max-w-[40rem] lg:max-w-[50rem]">
        <Card>
          <CardContent className="flex flex-col gap-4 p-2!">
            <FormControl className="MuiTextField-root relative mb-0 w-full">
              <TextareaAutosize
                minRows={2}
                maxRows={3}
                className="MuiInputBase-root MuiInput-root outlined autosize bg-background-paper! w-full resize-none pr-28! outline-none!"
                placeholder="Chat with the AI assistant..."
                onChange={handleInputChange}
                onKeyDown={handleInputKeyDown}
                value={inputValue}
              />
              <Box className="absolute right-0 flex flex-row">
